...
export class ListComponent implements OnInit {
  ...
  private readonly exportConfig = {
    ...
    title: 'Listado de Subcategorías',                // Título del reporte
    filename: 'Subcategorias',                        // Nombre base del archivo

    // Columnas a exportar - CONFIGURA SEGÚN TUS DATOS
    columns: [
      { key: 'No', header: 'No.', width: 8, align: 'center' as const },
      { key: 'Descripción', header: 'Descripción', width: 25, align: 'left' as const },
      { key: 'Categoría', header: 'Categoría', width: 20, align: 'left' as const }
    ] as ExportColumn[],
    
    // Mapeo de datos - PERSONALIZA SEGÚN TU MODELO
    dataMapping: (modelo: Subcategoria, index: number) => ({
      'No': modelo?.No || (index + 1),
      'Descripción': this.limpiarTexto(modelo?.subc_Descripcion),
      'Categoría': this.limpiarTexto(modelo?.cate_Descripcion)
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: 'Inventario' },
      { label: 'Subcategorías', active: true }
    ];
    ...
    this.http.get<Subcategoria[]>(`${environment.apiBaseUrl}/Subcategoria/Listar`, {
      ...
      setTimeout(() => {
        ...
        datosConIndex.forEach((item, index) => {
          ...
          item.subc_Descripcion = item.subc_Descripcion || ''; // Asegurar que subc_Descripcion no sea undefined
        });
