/**
 * Componente para editar tipos de vendedores existentes
 * Permite modificar la información de un tipo de vendedor, valida cambios y muestra confirmaciones antes de guardar
 */
...
export class EditComponent implements OnChanges {
  ...
  obtenerListaCambios() {
    const cambios = [];
    if (this.tipoVendedor.tiVe_TipoVendedor?.trim() !== this.tipoVendedorOriginal?.trim()) {
      cambios.push({
        label: 'Tipo de Vendedor',
        anterior: this.tipoVendedorOriginal,
        nuevo: this.tipoVendedor.tiVe_TipoVendedor
      });
    }
    return cambios;
  }
  ...
  /** Datos del tipo de vendedor recibidos desde el componente padre para editar */
  @Input() tipoVendedorData: TipoVendedores | null = null;
  /** Evento emitido cuando el usuario cancela la operación de edición */
  @Output() onCancel = new EventEmitter<void>();
  /** Evento emitido cuando se guarda exitosamente el tipo de vendedor editado */
  @Output() onSave = new EventEmitter<TipoVendedores>();

 /**
   * Objeto que contiene los datos actuales del tipo de vendedor en edición
 ...
 tipoVendedor: TipoVendedores = {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['tipoVendedorData'] && changes['tipoVendedorData'].currentValue) {
      this.tipoVendedor = { ...changes['tipoVendedorData'].currentValue };
      this.tipoVendedorOriginal = this.tipoVendedor.tiVe_TipoVendedor || '';
  ...
  validarEdicion(): void {
    ...
    if (this.tipoVendedor.tiVe_TipoVendedor.trim()) {
      ...
      if (this.tipoVendedor.tiVe_TipoVendedor.trim() !== this.tipoVendedorOriginal) {
  ...
  /**
   * Guarda los cambios realizados en el tipo de vendedor
   * Realiza una petición PUT al API para actualizar el tipo de vendedor
  ...
  private guardar(): void {
    ...
    if (this.tipoVendedor.tiVe_TipoVendedor.trim()) {
      ...
      const tipoVendedorActualizar = {
        tiVe_Id: this.tipoVendedor.tiVe_Id,
        tiVe_TipoVendedor: this.tipoVendedor.tiVe_TipoVendedor.trim(),
        usua_Modificacion: getUserId(),
        tiVe_FechaModificacion: new Date()
      };

      // Realiza la petición PUT al API para actualizar el tipo de vendedor
      this.http.put<any>(`${environment.apiBaseUrl}/TiposDeVendedor/Actualizar`, tipoVendedorActualizar, {
        headers: {
        ...
        next: (response) => {
          this.mensajeExito = `Tipo de vendedor "${this.tipoVendedor.tiVe_TipoVendedor}" actualizado exitosamente`;
        ...
        error: (error) => {
          console.error('Error al actualizar tipo de vendedor:', error);
          this.mostrarAlertaError = true;
          this.mensajeError = 'Error al actualizar el tipo de vendedor. Por favor, intente nuevamente.';
