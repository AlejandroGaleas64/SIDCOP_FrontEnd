/**
 * Componente para mostrar los detalles de un tipo de vendedor
 * Muestra información detallada de un tipo de vendedor seleccionado y permite cerrar la vista
 */
...
export class DetailsComponent implements OnChanges {
  ...
  /** Datos del tipo de vendedor recibidos desde el componente padre */
  ...
  /** Almacena los detalles del tipo de vendedor a mostrar */
   ...
   * Se ejecuta cuando tipoVendedorData cambia y carga los detalles del tipo de vendedor
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['tipoVendedorData'] && changes['tipoVendedorData'].currentValue) {
      this.cargarDetallesSimulado(changes['tipoVendedorData'].currentValue);
    }
  }

  /**
   * Simula la carga de detalles de un tipo de vendedor
   * Copia los datos recibidos y maneja posibles errores durante el proceso
   * @param data Objeto con los datos del tipo de vendedor a cargar
   */
  // Simulación de carga
  cargarDetallesSimulado(data: TipoVendedores): void {
    this.cargando = true;
    ...
    setTimeout(() => {
      try {
        ...
        this.tipoVendedorDetalle = { ...data };
        this.cargando = false;
      } catch (error) {
        console.error('Error al cargar detalles del tipo de vendedor:', error);
        this.mostrarAlertaError = true;
        this.mensajeError = 'Error al cargar los detalles del tipo de vendedor.';
        this.cargando = false;
      }
