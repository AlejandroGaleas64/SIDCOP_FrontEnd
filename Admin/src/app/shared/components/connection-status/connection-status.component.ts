  ...
  template: `
    <div class="connection-status-container">
      <div class="connection-status-indicator">
        <div class="status-dot" [ngClass]="{'active': isLocalActive, 'available': isLocalAvailable, 'unavailable': !isLocalAvailable}"></div>
        <span class="status-text">Local</span>
      </div>
      <div class="connection-status-indicator">
        <div class="status-dot" [ngClass]="{'active': isRemoteActive, 'available': isRemoteAvailable, 'unavailable': !isRemoteAvailable}"></div>
...
export class ConnectionStatusComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscription = this.connectionService.currentApiUrl$.subscribe(url => {
      ...
      this.isRemoteActive = url === 'http://200.59.27.115:8091';
    });
  ...
  private updateConnectionStatus(): void {
    ...
    this.isRemoteActive = status.activeUrl === 'http://200.59.27.115:8091';
  }
}
